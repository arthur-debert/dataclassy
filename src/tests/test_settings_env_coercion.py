...
def test_env_dict_key_value_pairs():
    ...
    os.environ["TEST_HEADERS"] = "Content-Type=application/json,Authorization=Bearer token"
...
def test_env_dict_json_format():
    ...
    os.environ["TEST_METADATA"] = '{"version": "1.0", "debug": true, "count": 42}'
...
def test_env_empty_collections():
    ...
    class Config:
        ...
        metadata: Dict[str, str] = field(default_factory=lambda: {"default": "value"})
...
def test_env_complex_nested():
    ...
    os.environ["APP_SERVERS"] = "api.example.com,web.example.com,admin.example.com"
    ...
    try:
        ...
        assert config.servers == ["api.example.com", "web.example.com", "admin.example.com"]
        assert config.ports == [8080, 8081, 8082]
        assert config.features == {"auth": True, "logging": True, "caching": False}
        assert config.limits == {"max_requests": 1000, "timeout": 30, "retries": 3}
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            loaded = Config.load_config(config_paths=[config_path], load_env=False)
            assert loaded.servers == config.servers
            assert loaded.features == config.features
    finally:
        for key in ["APP_SERVERS", "APP_PORTS", "APP_FEATURES", "APP_LIMITS", "APP_ADMIN_EMAIL"]:
...
def test_env_dict_with_equals_in_value():
    ...
    os.environ["TEST_CONNECTION_STRINGS"] = "db=host=localhost;port=5432,cache=redis://localhost:6379"
