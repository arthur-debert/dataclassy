...
@pytest.mark.parametrize(
    "enum_class,input_value,error_msg",
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"a": 1, "b": 2}, {"a": {"x": 10}}, "deep", {"a": {"x": 10}, "b": 2}),
        # Deep merge - override dict with non-dict
        ({"a": {"x": 1}, "b": 2}, {"a": 42}, "deep", {"a": 42, "b": 2}),
        # Empty dicts
        ({}, {"a": 1}, "deep", {"a": 1}),
        ({"a": 1}, {}, "deep", {"a": 1}),
        ({}, {}, "deep", {}),
...
@pytest.mark.parametrize(
    "field_type,input_value,expected_value,expected_type",
    [
...
def test_union_type_handling():
    ...
    data1 = {"value": 42, "optional_middle": None, "nested": [1, 2, 3]}
...
def test_inheritance_with_from_dict():
    ...
    data = {"base_field": "base", "derived_field": "derived", "shared": 42}
